...
AMT_Window:SetSize(960, PVEFrame:GetHeight())
...
AMT_Window:SetScript("OnShow", function()
	...
	for i = 1, #AMT.PVEFrame_Panels do
		if AMT.PVEFrame_Panels[i].isVisible then
			tinsert(VisiblePanels, {
				text = AMT.PVEFrame_Panels[i].text,
				frameName = AMT.PVEFrame_Panels[i].frameName,
				isVisible = AMT.PVEFrame_Panels[i].isVisible,
			})
		end
	end
